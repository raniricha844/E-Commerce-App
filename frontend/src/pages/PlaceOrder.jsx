...
const PlaceOrder = () => {
  ...
  const {navigate,cartItems,setCartItems,delivery_fee,products,backandUrl,token,getCartAmount} = useContext(ShopContext);
  ...
  const initPay = (order) => {
    const options = {
      key: import.meta.env.VITE_ROZERPAY_KEY_ID,
      amount: order.amount,
      ...
      handler: async(response) => {
        ...
        try{
          ...
          const {data} = await axios.post(backandUrl + '/api/order/verifyRazorpay' , response,{headers:{token}})
          if(data.success){
            navigate('/orders')
            setCartItems({})
          }
  ...
  const onSubmitHandler = async (event) => {
     ...
     try{
      ...
      switch(method){
        ...
        case 'cod':
          const response = await axios.post(backandUrl + '/api/order/place' , orderData , {headers:{token}}) 
        ...
        case 'stripe':
           ...
           const responseStripe = await axios.post(backandUrl + '/api/order/stripe',orderData,{headers:{token}})
      ...
      case 'razorpay' :
        const responseRazorpay = await axios.post( backandUrl + '/api/order/razorpay' ,orderData,{headers:{token}}) 
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
        ...
        <div className='flex gap-3'>
          <input required onChange={onChangeHandler} name='zipcode' value={formdata.zipcode} className=' border border-gray-300 py-1.5 px-3.5 w-full'type="number" placeholder='Zip Code'/>
